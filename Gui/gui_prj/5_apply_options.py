...
def merge_image():
    try:
        ...
        img_sizes = [] # [(width1, height1),(width2, height2),...]   width1 = cmb_width , height1 = cmb_height , width2 = img_width , height2 = ?
        if img_width > -1:
            img_sizes = [(int(img_width), int(x.size[1]) * img_width / int(x.size[0])) for x in images]           
        else:
            #원본사이즈 사용
            img_sizes = [(int(x.size[0]), int(x.size[1])) for x in images]
        ...
        # 포멧 옵션 처리
            file_name = "Dang_photo." + img_format 
